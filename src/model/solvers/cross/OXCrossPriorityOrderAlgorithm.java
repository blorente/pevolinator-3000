...
public class OXCrossPriorityOrderAlgorithm extends CrossAlgorithm {
	...
	List<Individual> crossPair(Individual ind, Individual selected) {
		...
		List<Integer> nums = this.getCrossPointsList(genomeSize + 1, numPoints);
	...
	private void performOXPriorityOrderCross(Individual ind, Individual selected, List<Integer> points) {
		...
		fixGenomePrioOrder(firstChild, ind.getGenome(), selectedPositions, genomeSize);
		fixGenomePrioOrder(secondChild, selected.getGenome(), indPositions, genomeSize);
	...
	private void fixGenomePrioOrder(Gene[] child, Genome genome, List<Integer> positions, int genomeSize) {
		int i = 0;
		int j = 0;
		while(i < genomeSize){
			if(child[i] == null){
				child[i] = genome.getGene(positions.get(j));
				j++;
			}
			i++;
		}		
